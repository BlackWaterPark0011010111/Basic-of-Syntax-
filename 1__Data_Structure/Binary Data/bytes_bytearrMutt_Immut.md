...
Байты сами по себе -это те данные,что уже свершились, это то ,что уже завершено и попытка его исправить или подправить будет разрушением подобного гаранта подлинности,самой сути существования байта.если нам нужно прочитать файл в бинарном режиме('rb'),это будет слепок  тех байтов,которые находились на диске в микроточный момент времени открытия файла. или когда сокет присылает сетевой пакет это bytes,мгновенный снимок того, что пролетело по каналу связи к которым нельзя прикасаться.
...
А bytearray уже как раз изменяемая последовательность байтов,это уже будет как список,но только для чисел от 0 и до 255(атомарный уровень), 2 в 8 степени минус 1. То есть мы в нем можем,менять,добавлять и удалять элементы. 
...
байт это минимальная адресуемая единица информации в числе от 0 до 255, то есть Bytes и bytearray это последовательности таких чисел и интерпретируются как сырые байты а не как символы текста. они оба схожие конструкторы,просто Bytes дает неизменяемый обьект а bytearray изменяемый,но каждый свой отдельный тип способа осуществления раскрывает свою суть типа:
если создать обьект заданной длинны и заполнить его нулевыми байтами как bytes(5),то он создаст последовательность b'\x00\x00\x00\x00\x00' и здесь \x00 будет шестнадцатеричным представлением нуля.Потом способ создания из итерируемого обьекта  целых чисел в диапазоне 0---255: bytes([65,66,67])--будет b'ABC', а если выйти за пределы то вызывается ошибка ValueError,которая подчеркивает саму привязанность к 8-ми битному представлению. и еще создание обьекта из другого который будет поддерживать протокол будет операцией копирования памяти.конструктор bytes не анализирует содержимое и он не кодирует символы,он лишь повторяет последовательность байтов из одной области в другую. также четвертый способ,через копирование с указанием самой кодировки:bytes('blabla', encoding='utf-8')-это будет преобразовывать unicode строку в последовательность байтов исходя из заданной кодировки. то есть тут мы явно говорим,что нужно преобразовать эту строку в байты для будущей записи в файл или отправки по сети и тд. bytearray() он поддерживает все те же варианты конструктора но результат поводить через само присваивание  по индексу:
arr = bytearray(b'ABC');
arr[0] = 88
и выведет b'XBC'
...
это английский язык и написав кириллицей без указания кодировки,будет просто ошибка,потому что с тех пор таблицы ascii расширялись и стали использовать 8 бит-256 значений. ждя не-ASCII символов есть escape-последовательности,это шестнадцатеричные(\x41 для 'A'),восьмеричные(\o101) и управляющие символы(\n, \t).
и для русского языка байтовый литерал 'привет' в кодировке Utf-8,может быть записан только через шестнадцатеричную систему кода,вот:b'\xd0\xbf\xd1\x80\xd0\xb8\xd0\xb2\xd0\xb5\xd1\x82'.

Протокол буфера(Buffer Protocol)
С-интерфейс Python  и он открывает доступ к внутреннему представлению обьекта и bytes и bytearray  будут реализаторами протокола. при создании bytes в памяти выделяется регион куда записываются значения. bytearray работает аналогично но с возможностью усовершенствования этого региона. Сам буфер обеспечивает нулевое копирование(zero copy) при передаче данных между компонентами,то есть данные не дублируются при передаче из одного места в другое а передается лишь ссылка на блок в памяти с инфо о размере и формате. 
Memoryview -это обьект который предоставляет интерфейс для доступа к самому протоколу буфера. он создает окно которое дает сам вид с параметрами на часть данных и все операции через это окно происходят без создания копий в процессе. 

переполнение буфераTODO:"19 смертных грехов, угрожающих безопасности программ"
стек растет сверху вниз,от старших адресов к младшим(схема адресации 'little-endian')это означает,что младший байт хранится в памяти первым и адрес возврата '3f104000' означает 0х0040103f. когда буфер переполняется вслед за buf находится сохраненное значение регистра EBP(Extended Base Pointer-расширенный указатель на базу).EBP содержит указатель кадра стека, при ошибке на одну позицию его значение будет затерто,то есть перезаписано
,а потом и адрес возврата,и потом может произойти перенаправление в выполнения на shell-code.уязвимости затрагивают также аргументы функции,указатели на функции или виртуальные таблицы.си так же для кучи(heap другая область памяти,растет вверх),где данные хранятся рядом с пользовательскими.создавать эксплойты!!

Преобразование из\в списки чисел и ихнее внутреннее представление. 
Мы преобразуем сами числовые значения, которые уже лежат в объекте bytes
изначально если нам нужно преобразовать данные в список,как здесь(b = b'ABC'  list(b)),то мы  сразу работаем не с обработкой ABC или какими-то иными любыми символами что задаем в b'', а работаем с ячейками в ascii таблице.мгновенно. то есть мы работаем не с символами, а с числовыми адресами-кодами
для Python b'ABC'  это всегда один и тот же набор байтов.
в list(b'ABC') - мы говорим что хотим видеть числа 65,66,67, а в bytes([65, 66, 67])  -- говорим что хотим видеть ABC.
нр если мы эти оба действия присвоим переменной 'x и y' и через  is проверим это одно и то же в памяти,то будет false,потому что это разные объекты в памяти, но с ОДИНАКОВЫМ содержимым

x = b'ABC'             1 ячейка  1ячейка  1ячейка  
y = bytes([65,66,67]) # 1 байт    1байт    1байт
obj1 == obj2# true они абсолютно одинаковые и никогда не изменятся,они статичны.
obj1 is obj2# а здесь false
из-за неизменяемости bytes,Python может проводить кеширование часто используемых литералов, поэтому b'ABC' is b'ABC' может быть истинным, но рассчитывать на оптимизацию в логике программы нельзя.



если писать b'ABC',python не хранит буквы 'A', 'B', 'C'. Он хранит три числа: 65, 66, 67.Это байты и отображение b'ABC' в консоли -это всего лишь услуга интерпретатора, которая для удобства показывает байты, значения которых попадают в диапазон печатаемых ASCII-символов, в виде этих символов. Если бы там было число 200,он показал бы \xc8

list(b'ABC') и bytes([65, 66, 67])-это два взгляда на одну и ту же сущность, числовую последовательность в памяти компьютера.

значение в ячейки не могут быть отрицательными- Байт-это 8 бит.и когда мы получаем значение через индекс b[0], оно представляется как int, и его значение всегда будет в диапазоне от 0 до 255. Отрицательным оно быть не может, потому что байт физически не может хранить отрицательное число.если создать bytes([-1]),выдаст ошибку ValueError,-1 не влезает в байт.
это показатель такого механизма,который помогает модифицировать, генерировать бинарные данные заголовков алгоритмически,если например преобразование между байтами и списками -это управление бинарными данными.список чисел как например [0x12, 0xAB, 0x00]), это логичная  структура для проектирования и для проверки  самой последовательности,но для производительности и расхода в памяти в обычном списке каждый его элемент это полноценный int который будет занимать десяток байтов и bytearray превратит этот список в готовый к отправке бинарный буфер сразу и изменяемый bytearray предоставляет сам интерфейс который аналогичный списку пошаговой сборки таких данных,методы arr.append(68), arr.extend([69,70]),arr.insert(0, 64) будут работать именно с целыми числами структурируя последовательность. также в отличии от списка bytearray наследует методы строк(некоторые) как upper() или find(). эти методы работают опираясь на ASCII-стандартную таблицу,потому что b'abc'.upper() возвращает b'ABC',преобразовав коды строчных букв в коды прописных (97 в 65),а для bytearray методы типа upper() модифицируют объект на месте.
типы bytes bytearray реализованы как обертки над массивом и для ключевого механизма передачи данных между модулями ,библиотеками без копий будет протокол буфера.
если обращаемся по индексу b[0]---возвращается целое число,там 65,а при взятии среза b[0:1]---- возвращает новый байтовый обьект b'A' bytea или bytearray.b[0] это число и b[0:1] уже байтовая строка и объекты разных типов они не равны. нужно через ord.! if b'ABC'[0] == ord(b'A'):
внутренняя механика bytearray как и List,bytearray будет выделять блок памяти с запасом и операции append,extend будут выполнятся быстро,но при исчерпании запаса будет выделятся больший блок и уже туда копироваться все данные и операция insert в середину будет требовать сдвига всего хвоста массива
...
Для сериализации бинарных протоколов,кроме struct,для сложных форматов есть библиотеки protobuf,msgpack, flatbuffers.Они превращают словари,списки в плотную сжатую пачку байтов,двоичный формат и это не то что можно прочесть своими глазами, и итог для ихней работы будет  объект bytes, готовая пачка байтов для отправки или сохранения.Сам словарь это картотека, мы кладем в ключи те значения, которые считаем, что никогда не будут изменены.и если бы все записи bytes,все переданные данные могли внезапно изменится посреди вычислений,мы бы получили бессмысленность. например поиск \r\n\r\n в HTTP-заголовках,мы анализируем байты которые получили и если бы они могли изменится, тогда где гарант того,что найденный маркер-это тот маркер который нам нужен,который был в сходном пакете? а bytes дает уверенность в том что мы анализируем-это точная копия того что пришло.
...
ord() функция она работает мостом между текстом и числами, но работает на стороне строк.
ord('A') будет возвращать кодовую точку число с ASCII кодом 65 и при обращении к байтовому объекту по индексу(b'A'[0]) тоже получим 65.эти значения могут совпадать, но происхождение разное. ord оперирует абстрактными символами Unicode а индексация bytes сырыми данными.
