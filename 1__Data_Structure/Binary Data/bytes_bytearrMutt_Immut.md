...
это показатель такого механизма,который помогает модифицировать, генерировать бинарные данные заголовков алгоритмически,если например преоборазование между байтами и списками -это управление бинарными данными.список чисел как например [0x12, 0xAB, 0x00]),будет идеальной структурой для сборки и проверки и преобразование bytearray([...]), первратит этот список в готовый к отправке бинарный буфер сразу и изменяемый bytearray предоставляет сам интерфейс котоырй аналогичный списку пошаговой сборки таких данных,методы arr.append(68), arr.extend([69,70]),arr.insert(0, 64) будут работать именно с целыми числами структурируя последовательность. также в отличии от списка bytearray наследует методы строк(некоторые) как upper() или find(). эти методы работают опираясь на ASCII-стандартную таблицу,потому что b'abc'.upper() возвращает b'ABC',преобразовав коды строчных букв в коды прописных (97 в 65),а для bytearray методы типа upper() модифицируют объект на месте.
bytes bytearray- реализованы как обертки над непрерывным массивом байтов и хранение будет вполне эффективно для памяти и скорости и без копирования через протокол (buffer protocol).
